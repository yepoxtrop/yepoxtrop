# <img src="./tux.png" width="50"/> Hola, soy Luis Díaz (Yexpotrop)

🎓 Estudiante de Ingeniería de Sistemas (4° semestre)  
💻 Desarrollador Full Stack en formación  

Actualmente me enfoco en el desarrollo de aplicaciones web, con experiencia en:
- Desarrollo de software  
- Automatización de procesos  
- Soporte técnico  

Cuento con ~1 año de experiencia construyendo soluciones reales.  
🚀 Busco seguir creciendo como **Fullstack Developer** y en el área de **Infraestructura y DevOps**.

---

## 🚀 Tecnologías

### ⚙️ Backend
![Python](https://img.shields.io/badge/Python-Backend-blue?logo=python&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-Microframework-black?logo=flask)
![FastAPI](https://img.shields.io/badge/FastAPI-Modern_API-009688?logo=fastapi&logoColor=white)
![Django](https://img.shields.io/badge/Django-Web_Framework-092E20?logo=django&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-Backend-339933?logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-Minimal_Framework-black?logo=express)
![C%23](https://img.shields.io/badge/C%23-.NET-512BD4?logo=csharp&logoColor=white)
![Java](https://img.shields.io/badge/Java-Spring_Boot-6DB33F?logo=springboot&logoColor=white)

---

### 🎨 Frontend
![JavaScript](https://img.shields.io/badge/JavaScript-Frontend-F7DF1E?logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-Typed_Javascript-3178C6?logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-UI_Library-61DAFB?logo=react&logoColor=black)

---

## 📈 Actualmente aprendiendo
- Arquitectura de software  
- DevOps & CI/CD  
- Infraestructura (Docker, Linux, redes)  

---

## 📫 Conecta conmigo
- GitHub: https://github.com/yepoxtrop
- LinkedIn: www.linkedin.com/in/luis-angel-sarmiento-díaz-77265a317
